...
export default function CategoryManagement({ navigation }) {
    ...
    const [categoryItems, setCategoryItems] = useState([]); // Danh sách sản phẩm
    const [isModalVisible, setModalVisible] = useState(false); // Trạng thái của Modal
    const [selectedCategoryId, setSelectedCategoryId] = useState(null); // Lưu ID danh mục cần xóa
    const [selectedCategoryName, setSelectedCategoryName] = useState(''); // Lưu tên danh mục cần xóa
        ...
        const unsubscribe = firestore()
            ...
            .onSnapshot(snapshot => {
                const categories = snapshot.docs.map(doc => ({
                    id: doc.id,
                    ...doc.data(), // Lấy toàn bộ data của mỗi document
                }));
                setCategoryItems(categories); // Cập nhật state với dữ liệu mới nhất
            }, error => {
    ...
    // Hàm mở Modal và lưu lại ID, tên danh mục cần xóa
    const showDeleteModal = (id, name) => {
        setSelectedCategoryId(id);
        setSelectedCategoryName(name); // Lưu tên danh mục cần xóa
    ...
    const hideModal = () => {
        ...
        setSelectedCategoryId(null); // Reset lại danh mục đã chọn
        setSelectedCategoryName(''); // Reset lại tên danh mục
    };
    ...
    // Hàm xóa danh mục từ Firestore
    const handleDeleteCategory = async () => {
        if (selectedCategoryId) {
            try {
                await firestore().collection('DanhMuc').doc(selectedCategoryId).delete();
                console.log('Danh mục đã được xóa!');
                showAlert('Thành công', `Danh mục "${selectedCategoryName}" đã được xóa!`);
            } catch (error) {
    ...
    const filteredProducts = categoryItems.filter(product =>
        product.tenDanhMuc.toLowerCase().includes(searchText.toLowerCase())
    );
    ...
    const renderItem = ({ item }) => (
        <View style={styles.productItem}>
            {item.urlAnh ? (
                ...
                <Image source={require('../../assets/images/default-images.jpg')} style={styles.productImage} />
            )}
            <View style={styles.productDetails}>
                <Text style={styles.productName}>{item.tenDanhMuc}</Text>
            ...
            <View style={styles.actionButtons}>
                <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate('AddCategoryScreen', { category: item })}
                >
                    <FeatherIcon name="edit" size={24} color="#007bff" />
                </TouchableOpacity>
                <TouchableOpacity style={styles.deleteButton} onPress={() => showDeleteModal(item.id, item.tenDanhMuc)}>
            ...
            >
                <View style={styles.modalBackground}>
                    <View style={styles.modalContainer}>
                        ...
                        <Text style={styles.modalMessage}>Bạn có muốn xóa danh mục "{selectedCategoryName}" này?</Text>
